...
def find_homography(src: NDArray, dst: NDArray) -> NDArray:
    ...
    _, _, Vt = np.linalg.svd(A)  # Compute the SVD of A
    h = Vt[-1].reshape((3, 3))  # homo matrix is the last column
...
def draw_lines(img: MatLike, *lines: tuple[int, int]) -> MatLike:
    ...
    for l in lines:
        ...
        if B == 0:  # Vertical line
            ...
            assert len(intersections) == 2, 'Line must be in the image'
...
class HomographyFinder:
    ...
    def backward_warp(self, h: NDArray) -> MatLike:
        ...
        # Calculate the bounds and the weights of the target coordinate
        ...
        interpolated_values = (
            ((1 - wx) * (1 - wy)).reshape(-1, 1) * self.img_src[y0, x0]
            + (wx * (1 - wy)).reshape(-1, 1) * self.img_src[y1, x0]
            + ((1 - wx) * wy).reshape(-1, 1) * self.img_src[y0, x1]
            + (wx * wy).reshape(-1, 1) * self.img_src[y1, x1]
        )
